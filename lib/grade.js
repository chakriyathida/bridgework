...
function gradeBlocks(step, value) {
  const script = Array.isArray(value) ? value : [];
  if (script.length === 0) {
    return { status: "missing", score: 0, note: "Drag some blocks into your script to submit this step." };
  }
  const want = step.answer || [];

  if (script.length === want.length && script.every((b, i) => b === want[i])) {
    return { status: "correct", score: 1, note: "That program runs correctly." };
  }

  // How far does it run before it goes wrong? That's the useful thing to tell them.
  let ok = 0;
  while (ok < script.length && ok < want.length && script[ok] === want[ok]) ok++;

  const sameSet =
    script.length === want.length &&
    [...script].sort().join("|") === [...want].sort().join("|");

  if (sameSet) {
    return {
      status: "partial",
      score: 0.5,
      note: `Right blocks, wrong order. Your program does the first ${ok} step${ok === 1 ? "" : "s"} correctly, then goes wrong.`,
    };
  }
  if (script.length < want.length) {
    return {
      status: "partial",
      score: ok / want.length >= 0.5 ? 0.5 : 0,
      note: `Your script is missing ${want.length - script.length} block${want.length - script.length === 1 ? "" : "s"}. It runs, but it stops early.`,
    };
  }
  if (script.length > want.length) {
    return {
      status: "partial",
      score: 0.5,
      note: `You have ${script.length - want.length} block${script.length - want.length === 1 ? "" : "s"} too many. A working program is usually the shortest one that does the job.`,
    };
  }
  return { status: "wrong", score: 0, note: "That program doesn't do what was asked. Read the model script below." };
}
...
export function gradeStep(step, value) {
  ...
  if (step.type === "blocks") return gradeBlocks(step, value);
